...
export default function CropWise(){
  ...
  const detectLocation=()=>{if(!navigator.geolocation){alert("Geolocation is not supported by your browser.");return;}setLocating(true);navigator.geolocation.getCurrentPosition(async(pos)=>{const{latitude:lat,longitude:lng}=pos.coords;try{const res=await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`);const data=await res.json();const a=data.address;const city=a.city||a.town||a.village||a.county||a.state_district||"";const state=a.state||"";setLocation([city,state].filter(Boolean).join(", "));}catch(e){setLocation(`${lat.toFixed(4)}, ${lng.toFixed(4)}`);}finally{setLocating(false);}},()=>{setLocating(false);alert("Location access denied. Please allow location permission.");});};
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24}}>
      <div>
        <div style={card}>
          ...
          <div style={{display:"flex",gap:10}}><input style={{...inp,flex:1}} placeholder="District, State" value={location} onChange={e=>setLocation(e.target.value)}/><button className="ob" style={{padding:"12px 20px",background:"#0f172a",border:"1px solid #4ade8044",borderRadius:12,fontSize:13,fontWeight:600,color:"#4ade80",cursor:"pointer",whiteSpace:"nowrap"}} onClick={detectLocation}>{locating?<span style={{animation:"spin 1s linear infinite",display:"inline-block"}}>⟳</span>:"📍 Use Current Location"}</button></div>
        </div>
        <div style={card}>
